...
1. **中國地方海事局 selector 仍待持續校準**：中央入口找不到「航行警告」導覽按鈕、浙江/上海解析 0 筆、海南 403 等，已在實機回報後修正錯誤分類（PARSE_ERROR/BLOCKED 不再誤判成 CONNECTION_ERROR）並補上地方來源的 debug 快照儲存機制，但實際 selector 仍需依下次執行產生的 `debug/` 快照持續調整。
