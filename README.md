...
1. **中國海事局各來源持續依實機回報校準中**：中央入口的網址、導覽按鈕、省級選單、警告列表容器（`.main_list_ul`）已依使用者提供的真實 HTML 修正並補上單元測試；福建/廣東/山東/遼寧/深圳改用中央入口選單裡確認存在的直連網址並已啟用（原本因為只有猜測子網域、從未驗證而預設停用）；浙江/上海/海南也從猜測子網域換成同樣確認過的直連網址。**detail 頁（點進單筆警告的內文頁面）結構仍未取得任何真實範例**，中央入口曾回報 320 筆列表全部 `PARSE_ERROR`（detail_success=0）——已將原本逐筆開/關新分頁的做法改成同分頁直接導航（懷疑是資源耗用問題），並讓單筆失敗不再讓整個來源掛掉、同時記錄實際錯誤訊息，但實際成效仍待下次執行驗證；若還是失敗，麻煩把 `debug/` 目錄下的快照或瀏覽器手動點開一筆警告的原始碼提供，才能確認 detail 頁結構。
2. 台灣航港局的「keep/drop 進入門檻」目前仍是**只看標題**（`check_keywords(title)`，並強制保留含「礙航」「射擊」的標題），detail 內文目前已改為會抓取並存入 `details` 欄位、實際參與風險評分與摘要，但尚未反過來影響「要不要保留這筆」的第一階段判斷；若要完全比照中國來源「先抓詳情、內文也能決定去留」，需另外排期處理。
3. 陸軍/空軍等純行政性質「演訓場域通報申請案/申請單」已加入 exclusion_patterns 大幅降權（非真正生效中的限航公告），真正的對海實彈射擊事件（報告單/通報）維持原風險等級；但此規則是以「申請案／申請單」等既有樣本中觀察到的字詞為依據，若航港局改用其他措辭，可能需要再擴充 exclusion_patterns。
